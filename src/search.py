...
class RAGSearch:
    def __init__(self, persist_dir="faiss_store", embedding_model="all-MiniLM-L6-v2", llm_model="llama-3.1-8b-instant"):
        ...
        if not self.vectorstore.load():
            ...
            self.vectorstore.update_from_new_documents("data")
    ...
    def search_and_summarize(self, query: str, top_k: int = 10) -> str:
        ...
        # STRICT PROMPT: No guessing, no bold headers, no filler
        # UPDATED FOR CLICKABLE LINKS
# STRICT SPOKESPERSON PROMPT
        prompt = f"""
